...
- Скачивание осуществленно при помощи модуля: «youtube_dl» и встроенного в него модуля 
(для упаковки/переупаковки файлов): «FFmpeg» 
- Для многопоточного скачивания файлов используяется библиотека «Thread»
- Работа с изображением происходит с помощью библиотеки «PIL»
- Вставка и редактирование тэгов в аудифайлах используется библиотеке: «mutagen»
...
название трека/исполнителя/все сразу/некоррекного ввода и тд
- Единовременно осуществлять n-ое количество скачек и в тоже время производить поиск
- Контролировать название файла, вдобавок автоматическое заполнение лишь основных аудиотэгов
...
Буду примного благодарен если на примере моего «кода» покажите как его на самом деле следовало 
написать (хотя бы пару примеров) особенно в том моменте где при парарельном скачивание файлов
прогресс проставлялся именно в том месте где это нужно

в конечном итоге время затраченное с момента запуска скрипта до момента финально готового файла - очень большое..
"""
...
class Window:
    ...
    def size_and_position(self, master, width=int(570), height=int(220)):
...
.
    ...
    def main_widget(self):
        ...
        Label(self.tab_1, text="button_download save_to_as",fg="#df0a0a", font=('calibri', 26, 'bold')).grid(row=0, columnspan=2, pady=10)
    ...
    def button_configure(self, check_list: list) -> object:
        """
        Функция получает список из славорей параметров, очищает родительский контейнер от прошлого запроса и  создав n колв.во кнопок с этими параметрами. Удаляет (если есть) существующие кнопки, добавляет новосозданные кнопки во фрейм :\n
        ...
        """
        # добавление счетчика для нумерации строк располажений кнопок скачивания
        count = 0
    ...
    def save_to_as(self, name, href):
        ...
        После, запуск в отдельном потоке, скачевание файла
    ...
    def my_hook(self, d: dict) -> object:
        ...
        В конкретном Frame, на одной строке, создает два Label (в одном записанно название файла, в другом прогресс выполнения)
    ...
    def add_tags(filepath: str, title: str) -> object:
        ...
        В начале преобразует скаченную вместе с аудиофалом картинку, из формата webp в формат jpg
        ...
        Тэгирует и вставляет преобразованную картинку в аудифайл, а после удаляет весь кэш
    ...
    def rename(title: str) -> str:
        """ 
        - Преобразут название удаляя из него лишние символы и слова 
    ...
    def input_validate(self, event):
        ...
        а после создание соответсвующих виджетов
        """
...
if __name__ == "__main__":
    window = Window(570, 220, "Download YouTube music.mp3")
    window.run()